...
def get_env_data(key):
    try:
        ...
        if data:
            ...
            logger.info(f"Данные о {key.lower()} успешно загружены из переменных окружения")
            return parsed
    except Exception as e:
        logger.error(f"Ошибка загрузки {key.lower()}: {e}")
    return []

def set_env_data(key, data):
    try:
        os.environ[key] = json.dumps(data, ensure_ascii=False)
        logger.info(f"Данные о {key.lower()} успешно сохранены в переменные окружения")
    except Exception as e:
        logger.error(f"Ошибка сохранения {key.lower()}: {e}")

def load_products_data():
    return get_env_data('PRODUCTS_DATA')

def save_products_data(data):
    set_env_data('PRODUCTS_DATA', data)

def load_hoz_data():
    return get_env_data('HOZ_DATA')

def save_hoz_data(data):
    set_env_data('HOZ_DATA', data)

def load_fish_data():
    return get_env_data('FISH_DATA')

def save_fish_data(data):
    set_env_data('FISH_DATA', data)
...
def init_db():
    ...
    fish_data = ["Филе форели охл."]

    save_products_data(default_products)
